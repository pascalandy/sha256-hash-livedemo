...
Total Input             34,998.00079631 BTC
Total Output            34,997.98534631 BTC
...
for NONCE in $(seq $MIN $MAX); do
    ...
    # Isolate the firsts CHAR
    DIFFICULTY="00"
    NOUCE_COMPARE=$(printf "$HASH_WITH_NOUCE" | cut -c1-2)

    # Compare zeros based on difficulty
    if [[ $HASH_WITH_NOUCE = "$DIFFICULTY"* ]]; then
        echo
        ...
        #echo "$NOUCE_COMPARE is not matching $DIFFICULTY"; echo;
        #try again
    fi
